// debug_page.js - Debug what's actually on the page
const { chromium } = require('playwright');

async function debugPage() {
  console.log('🔍 Debugging what's on the page...');
  ...
  try {
    ...
    for (let i = 0; i < Math.min(clickableCount, 10); i++) {
      try {
        ...
        console.log(`Clickable ${i + 1}: <${tagName}> "${text?.substring(0, 50)}" class="${className}"`);
      } catch (e) {
