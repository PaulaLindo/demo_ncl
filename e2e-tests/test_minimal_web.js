// e2e-tests/test_minimal_web.js - Test minimal Flutter web app
const { chromium } = require('playwright');

const BASE_URL = 'http://localhost:8089';

async function testMinimalWeb() {
  console.log('🔬 TESTING MINIMAL FLUTTER WEB');
  console.log('==============================');
  ...
  try {
    ...
    // Take screenshot
    await page.screenshot({ path: 'test-results/minimal_web_test.png', fullPage: true });
    
    // Final diagnosis
    if (sceneHostExists && hasTestContent) {
      console.log('✅ SUCCESS: Flutter web rendering works!');
      ...
      console.log('❌ ISSUE: Flutter loads but no scene host');
    } else {
...
testMinimalWeb().catch(console.error);
